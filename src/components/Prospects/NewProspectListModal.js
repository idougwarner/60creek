...
const STEP3 = 2;
const STEP4 = 3;
const NewProspectListModal = ({ show, close }) => {
  ...
  const pushData = async () => {
    try {
      setCompleted(false);
      const listInfo = await API.graphql(graphqlOperation(createProspectList, {
        input: {
          userId: user.id,
          name: listName,
          enhance: enhance
  ...
  useEffect(() => {
    if (!loading && listName && prospectList.length > 0) {
      setIsNext(true);
    } else {
      setIsNext(false);
    }
  }, [loading, listName, prospectList]);
  useEffect(() => {
    let errCounts = 0;
        ...
        <Modal.Body>
          {step === STEP1 && <div className="step-1">
            <Form.Group >
              ...
              <Form.Control type="text" placeholder="Enter Prospect List Name"
                value={listName}
