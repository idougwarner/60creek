  ...
  const loadData = async () => {
    if (user) {
      ...
      if (rt?.data?.listProspectLists?.items) {
        setList(rt.data.listProspectLists.items.map(item => ({ value: item.id, label: item.name })));
      }
    }
  }
  return (
    <>
      <Modal show={show} onHide={() => close({ data: completed })} size={step === STEP2 ? "xl" : "md"}>
        <Modal.Header >
          ...
          <Modal.Title>
            {step === STEP1 ? "Add To Existing List" : step === STEP2 ? "Prosect Created" : step === STEP3 ? "Confirmation" :
              <>
